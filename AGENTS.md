...
Do not modify anything in the `trained_agents/` folder or this `AGENTS.md` file.
...
Read the Markdown files in `history/` to see what previous iterations did and their results. Use that context to make changes that increase the score: maximize avg_reward over the last 100 episodes as much as possible while minimizing the number of steps taken per episode.
